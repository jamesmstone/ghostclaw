...
class WhatsAppChannelPlugin final : public IChannelPlugin {
public:
  explicit WhatsAppChannelPlugin(
      std::shared_ptr<providers::HttpClient> http_client = std::make_shared<providers::CurlHttpClient>());
  ~WhatsAppChannelPlugin() override;
...
private:
  ...
  [[nodiscard]] bool is_allowed_number(const std::string &number) const;
